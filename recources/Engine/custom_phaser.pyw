...
def launch_normal_mods(base_dir):
    ...
    # Pass the selected modpack, not the launcher root.  The engine uses the
    # modpack's metadata and JARs to select the Minecraft version.
    run_engine(engine_path, [os.path.abspath(base_dir)])
...
def run_custom(base_dir):
    ...
    launch_version = main_data["versions"]
    if isinstance(launch_version, (list, tuple)):
        if len(launch_version) != 1:
            raise Exception(
                "Custom .main.lmc 'versions' must contain exactly one "
                "Minecraft version."
            )
        launch_version = launch_version[0]

    if not isinstance(launch_version, str) or not launch_version.strip():
        raise Exception(
            "Custom .main.lmc 'versions' must name the Minecraft version "
            "to launch."
        )
    launch_version = launch_version.strip()
    ...
    # Pass base_dir to engine; engine itself resolves the Minecraft version
    # declared by this modpack.
    run_engine(engine_path, [os.path.abspath(base_dir), launch_version])
